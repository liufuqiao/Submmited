...
public class SitesAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public static final int TITLE = 0;//标题 跨一列 也就是合并两列
    public static final int ITEM = 1;//不跨列
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        switch (viewType) {//根据viewtyupe来区分，是标题还是数据项
            case TITLE://标题，加载显示标题的item布局，就一个textview显示文本，这里我们自顶一个标题的viewholder->SitesHolder
                ...
                return sitesHolder;
            case ITEM://数据项，雷同不赘述了，标题和数据项的item布局和veiwholder都不会相互影响的
    ...
    public int getItemViewType(int position) {
        ...
        if (items.get(position) instanceof Menus) {//根据items数据类型的不同来判断他是标题还是数据项
            return TITLE;//标题
        } else if (items.get(position) instanceof Menus.MenuItem) {
            return ITEM;//数据项
        } else {
            return -1;
        }
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, int position) {
        ...
        switch (holder.getItemViewType()) {
            case TITLE://标题
                ...
                sitesHolder.name.setText(((Menus) items.get(position)).getName());
                break;
            case ITEM://数据项
                SiteHolder siteHolder = (SiteHolder) holder;
                siteHolder.name.setText(((Menus.MenuItem) items.get(position)).getName());
                break;
        }
    }
