...
public final class SharedPreferencesHelper {
    ...
    private static final String preferenceName = "list";
     ...
     * @return
     */
    public static <T> List<T> getDataList(Context context, String tag, Class<T> clx) {
        List<T> datalist = new ArrayList<T>();
        String strJson = context.getSharedPreferences(preferenceName, Context.MODE_PRIVATE).getString(tag, null);
        if (null == strJson) {
            return datalist;
        }
        ...
        try {
            for (JsonObject jsonObject : jsonObjects) {
                arrayList.add(new Gson().fromJson(jsonObject, clx));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
